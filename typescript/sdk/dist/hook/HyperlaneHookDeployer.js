import { CCIPHook__factory, IL1CrossDomainMessenger__factory, StaticAggregationHook__factory, } from '@hyperlane-xyz/core';
import { ZERO_ADDRESS_HEX_32, addBufferToGasLimit, addressToBytes32, deepEquals, rootLogger, } from '@hyperlane-xyz/utils';
...
export class HyperlaneHookDeployer extends HyperlaneDeployer {
    ...
    async deployCCIPHook(chain, config) {
        const hook = this.ismFactory.ccipContractCache.getHook(chain, config.destinationChain);
        if (!hook) {
            this.logger.error(`CCIP Hook not found for ${chain} -> ${config.destinationChain}`);
            throw new Error(`CCIP Hook not found for ${chain} -> ${config.destinationChain}`);
        }
    ...
    async deployAmountRoutingHook(chain, config) {
        const hooks = [];
        ...
        // deploy routing hook
        const routingHook = await this.deployContract(chain, HookType.AMOUNT_ROUTING, [lowerHook, upperHook, config.threshold]);
        return routingHook;
    }
}
