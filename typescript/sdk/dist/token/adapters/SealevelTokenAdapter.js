...
export class SealevelTokenAdapter extends BaseSealevelAdapter {
    ...
    async populateTransferTx({ weiAmountOrId, recipient, fromAccountOwner, fromTokenAccount, }) {
        ...
        if (!fromAccountOwner)
            throw new Error('fromAccountOwner required for Sealevel');
        const tx = new Transaction().add(createTransferInstruction(new PublicKey(fromTokenAccount), new PublicKey(recipient), new PublicKey(fromAccountOwner), BigInt(weiAmountOrId)));
        return { transaction: tx, signers: [] };
    }
